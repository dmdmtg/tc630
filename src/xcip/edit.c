  ...
  register struct thing *t;	/* Thing to be edited */
{
  ...
  if (t != TNULL) {
	...
	if (d< (t->otherValues.radius>>1)) {
	  track(m,offset,MOVE,t); 
	}
	else {
	  track(m,offset,GROWCIRCLE,t);
	}
	...
	if (ptinrect(m,Rect(o.x+dx,o.y+dy,np.x-dx,np.y-dy))) {
	  if (t->border != SOLID) {
	   draw(t,offset);
	    xbox(raddp(t->bb,offset));
	  }
	  t->bb = moveBox(m,t->bb,offset,0);
	...
	if ((dx > dy) && (d > (t->otherValues.ellipse.wid>>2))) {
	  track(m,offset,GROWEWID,t); 
	}
	else {
	  if ((dx < dy) && (d > (t->otherValues.ellipse.ht>>2))) {
	    track(m,offset,GROWEHT,t); 
	  }
	  else {
	    track(m,offset,MOVE,t); 
	  }
	...
	if (distance(m,o)<distance(m,np)) {
	  track(np,offset,REVLINE,t);
	} else {
	  track(o,offset,LINE,t);
	}
	break;
      }
      case ARC: {
	...
	break;
      ...
      case TEXT: {
	/* Allow finer movements in text since bounding box in any case
	   will be increased to fit current alignment. */
	int saveAlignment;
	saveAlignment = currAlignment;
#ifdef FINEALIGN
	currAlignment = max(currAlignment>>1,1);
#else  /* FINEALIGN */
	currAlignment = max(currAlignment>>1,2);
#endif /* FINEALIGN */
	track(m,offset,MOVE,t);
	currAlignment = saveAlignment;
	break;
      }
      case SPLINE: {
	...
	if ((d == u-1) || (d==1)) {
	  xsegment
	      (add(offset,t->otherValues.spline.plist[(d==1)?1:(d-1)]),
	       add(offset,t->otherValues.spline.plist[(d==1)?2:d]));
	  np=track(t->otherValues.spline.plist[(d==1)?2:(d-1)],
	      offset,(d==1)?REVSPLINE:SPLINE,t); /* See note 1 in track.c */
	}
	else {
	  np=track2(offset,t->otherValues.spline.plist[d-1],
	              t->otherValues.spline.plist[d+1],
	              t->otherValues.spline.plist[d]);
	}
	xspline(offset,t->otherValues.spline.plist,u);
    ...
    if( eqpt(t->bb.origin,t->bb.corner) ) {
      /* edit thing has no size so remove it */
      t = deleteThing(t);
      thingSelected = 0;
      changeBrush(-1);
  ...
  struct thing *
copyThing(t,p,offset,drawit) 
  register struct thing *t; 	/* Thing to be copied. */
  Point p;			/* Relative location of things bb */
  Point offset;			/* Absolute location of bb. Used */
				/* only top draw the figure. */
  int drawit;			/* Draw thing if nonzero */
{
  ...
  if (t != TNULL) {
	...
	c->origin = add (t->origin, sub (p, t->otherValues.arc.start));
				/* This is needed because newArc */
				/* will compute the wrong origin */
				/* sometimes. */
	break;
      }
      case SPLINE: {
	...
	if ((c = newText(p,t->otherValues.text.s)) != TNULL) {
	  ...
	  c->otherValues.text.spacing = t->otherValues.text.spacing;
	  if ((c->otherValues.text.s = 
	      getSpace (strlen (t->otherValues.text.s) + 1 )) != NULL) {
	    strcpy (c->otherValues.text.s, t->otherValues.text.s);
	  }
	  else {
	    free (c);
	    c = TNULL;
