...
/* Note: tmenuhit is built into the DMD 630 ROMS. 
         For X11, using the menus program in LIBDMDX11.
*/
...
tmhit_top:
	 ...
	 */
	rhit = md->mr;
	rectclip(&rhit, physical.rect);
	md->b = balloc(rhit);
	Jcursinhibit();
	if(md->b)
		bitblt(&physical, rhit, md->b, rhit.origin, F_STORE);
	rectf(&physical, md->mr, (md->b ? F_CLR:F_XOR));
	if(md->b)
		rectf(&physical, inset(md->mr, BORDER-1), F_OR);
	rectf(&physical, inset(md->mr, BORDER), (md->b ? F_CLR:F_XOR));
	Jcursallow();
...
/* ->->-> */		goto PaintMenu;
	...
	if(md->b){
		...
		erasemenu(&mdata, t, hit, greyed);
		rectf(&physical, md->mr, F_XOR);
		rectf(&physical, inset(md->mr, BORDER), F_XOR);
	}
	...
	for(j=0, p=tro, i=md->top; i < min(md->top+md->lines, t->items); ++i, ++j){
		...
		if (t->iconwidth) {
			if(mi->icon)
			{
				...
				bitblt(mi->icon, mi->icon->rect, &physical, r,
					 (md->b ? F_OR:F_XOR));
			}
...
;
		string(mi->font, tfill, &physical, q, (md->b ? F_OR:F_XOR));
		if(mi->next)
		{
			bitblt(&B_rtarrow, B_rtarrow.rect, &physical,
	...
	register int hit;
{
	if(md->b) {
		Jcursinhibit();
		rectf(&physical, inset(md->mr, BORDER), F_CLR);
		Jcursallow();
	}
	else {
		if (!greyhit)
