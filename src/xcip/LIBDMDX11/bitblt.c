...
#ifdef X11
/*
 * This should work but doesn't - the color frame buffer currently
 * doesn't support using the planemask of the CG with XCopyArea
 * FIX IT when it works as described in the X manual
 */
#if 1
	XSetFunction(dpy, gc, f);
	if (f != F_STORE)
		XSetPlaneMask(dpy, gc, fgpix^bgpix);
	XCopyArea(dpy, sb->dr, db->dr, gc, r.origin.x, r.origin.y,
		wd, ht, p.x, p.y);
	if (f != F_STORE)
		XSetPlaneMask(dpy, gc, 0xFFFFFFFF);
#endif
#if 0
	if (f == F_STORE || f == F_OR) {
		XSetFunction(dpy, gc, f);
		...
		XCopyPlane(dpy, sb->dr, db->dr, gc, r.origin.x, r.origin.y,
			wd, ht, p.x, p.y, fgpix^bgpix);
...
Code f;
{
	...
	if (f == F_CLR) {
		...
		XSetFunction(dpy,gc,f);
		XSetForeground(dpy,gc,bgpix^fgpix);
	} else {
		XSetFunction(dpy,gc,f);
		XSetForeground(dpy,gc,fgpix);
	}
...
Code f;
{
	...
	if(b->flag & BI_OFFSCREEN)
		pr_replrop((Pixrect *)b->dr, r.origin.x, r.origin.y,
		   diff.x, diff.y, f, &textrect32, r.origin.x, r.origin.y);
	else
		pw_replrop((Pixwin *)b->dr, r.origin.x, r.origin.y,
		   diff.x, diff.y, f, &textrect32, r.origin.x, r.origin.y);
#endif /*SUNTOOLS*/
