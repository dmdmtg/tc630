...
struct obj *p;
{
	...
	if (c == '}') {
		...
		p->o_val[6] = q->o_val[6] = (int) stack[nstack+1].p_symtab;
...
int type;
{
	...
	p->o_ddval = ddval;
	p->o_attr = invis;
	dprintf("[] %d %d %d %d at %d %d, h=%d, w=%d\n", x0, y0, x1, y1, curx, cury, h, w);
	if (isright(hvmode))
		curx = x1;
	else if (isleft(hvmode))
		curx = x0;
...
struct obj *p;
{
	...
	dy = p->o_y - p->o_val[3];
	n = p->o_nobj + 1;
	dprintf("into blockadj: dx,dy=%d,%d\n", dx, dy);
	for (lev = 1; lev > 0; n++) {
		p = objlist[n];
		if (p->o_type == BLOCK)
			lev++;
			...
			lev--;
		dprintf("blockadj: type=%d o_x,y=%d,%d;", p->o_type, p->o_x, p->o_y);
		p->o_x += dx;
		p->o_y += dy;
		dprintf(" becomes %d,%d\n", p->o_x, p->o_y);
