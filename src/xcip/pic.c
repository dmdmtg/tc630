...
register struct thing *h;
{
  ...
  BBpic.origin.x = Xmax-Xmin;  BBpic.corner.x=0;
  ...
  Rectangle b;
{
  ...
  switch (t->type) {
	...
	/* single-line: use old method of simple text string. */
        ...
        /* Adjust text down a little - needed when printed  */
        adj = fontheight(t->otherValues.text.f->f) >>3;
	...
	if( strlen(fontString) == 2 )
        ...
        fprintf(f,"\\s%d\\&%s\\fP\\s0\" at %d,%d%s\n",
    ...
    case MACRO: {
      p = translate(t->origin,b);
      fprintf(f,"%s with .nw at %d,%d\n",
	         t->otherValues.list->name,p.x,p.y);
      break;
    }
  }
}
